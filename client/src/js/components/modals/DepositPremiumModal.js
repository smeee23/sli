...
class DepositPremiumModal extends Component {
  ...
  depositToChain = async(premiumDeposit, validatorId) => {
			...
			try{
				...
				result = await PremiumGeneratorAaveV2.methods.deposit(validatorId).send(parameter, async(err, transactionHash) => {
      ...
      <Fragment>
        ...
        <ModalCtas>
			...
			<div style={{marginLeft: "auto", marginTop:"auto", paddingBottom:"32px"}}>
          		<Button style={{marginLeft: "auto", marginTop:"auto"}} text="Deposit" callback={() => this.setAmount(depositInfo)}/>
		  	</div>
        </ModalCtas>
      </Fragment>
		);
